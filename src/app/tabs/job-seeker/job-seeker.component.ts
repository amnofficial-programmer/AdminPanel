...
import { JobSeekerService } from '../../service/apiservice/jobseeker.service';
import { JobSeekerModel } from '../../service/model/job-seeker.model';
...
export class JobSeekerComponent implements OnInit {
  ...
  jobSeekerViewData:JobSeekerModel;
  ...
  GetCustomerList() {
    debugger
    this.spinnerService.show();
    this.jobSeekerService.getJobSeekerList(2,1)
      .subscribe((response) => {
        this.spinnerService.hide();
        ...
        console.log(response);
        this.customerGrid = response['data'];
...
submitUpdateVerificationStatus(){
  ...
  this.spinnerService.show();
  const status=0;
  if(this.jobSeekerViewData.verificationStatus==="1"){
    this.status=1;
  }else{
    this.status=0;
  }
  this.jobSeekerService.updateVerifactionStatus(this.jobSeekerViewData.id, this.status).subscribe(res=>{
    this.spinnerService.hide();
    this.showNotification('top', 'right', 'success', 'Job Seeker Status Updated');
    // this.closedeleteForm.nativeElement.click();
  },err=>{
    this.spinnerService.hide();
    this.showNotification('top', 'right', 'danger', 'Error To Update Job Seeker Status');
  })
}
