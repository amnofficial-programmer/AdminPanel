...
export class CustomerDueReportComponent implements OnInit {
  ...
  ReportSubmit(){
    ...
    if(this.report.customerId == undefined || this.report.customerId == null || this.report.customerId == 0 )
    {
      ...
      this.report.customerId;
    }

    this.reportsService.getCustomerDueReport(this.report)
      .subscribe((response)=>
      {
       // console.log(response);
       this.showNotification('top','right','success','Search Successfully');
       console.log(response)
        this.customerDueData=response
        if ($.fn.dataTable.isDataTable('#customerDueTable')) {
          $('#customerDueTable').dataTable().fnClearTable();
          $('#customerDueTable').dataTable().fnDestroy();
        }
        this.totalDueAmount = 0;
        this.customerDueData.forEach((element,i)=> {
          this.totalDueAmount+= element.dueAmount;
        });
  ...
  public downloadPDF() {
    ...
    html2canvas(document.getElementById('content')).then(canvas=> {
      ...
      doc.addImage(img,'JPEG', 7, 6, 10, 14);
      doc.save('Report.pdf');
